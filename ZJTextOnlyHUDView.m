...
- (void)commonInit {
    ...
    self.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.60];
...
- (void)setText:(NSString *)text {
    ...
    const CGFloat widthpadding = 32.0f;
    const CGFloat heightpadding = 6.0f;
...
//    const CGFloat padding = 10.0f;
    ...
    frame.size.width = textRect.size.width + 2*widthpadding;
    if (frame.size.width > screenWidth) {
        frame.size.width = screenWidth - 2*widthpadding;
    }
    frame.origin.x = (screenWidth - frame.size.width)/2;
    frame.size.height = textRect.size.height + 2*heightpadding;
